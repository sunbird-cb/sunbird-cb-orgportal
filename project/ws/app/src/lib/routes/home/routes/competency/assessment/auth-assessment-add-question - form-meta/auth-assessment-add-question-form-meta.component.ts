...
import { ConfirmDialogComponent } from '../../quiz/shared/components/confirm-dialog/confirm-dialog.component'
...
export class AuthAssessmentAddQuestionFormMetaComponent implements OnChanges {
  ...
  async getRequestPayload() {
    ...
    if (questionConfig) {
      ...
      editiedMeta = {
        code: questionConfig[this.questionType].code,
        mimeType: questionConfig[this.questionType].mimeType,
        body: this.questionText,
        primaryCategory: questionConfig[this.questionType].primaryCategory,
        qType: (this.questionType === 'fitb') ? 'FTB' : toUpper(this.questionType),
        editorState: {
          options: optionsValue,
          question: this.questionText,
        },
        objectType: questionConfig[this.questionType].objectType,
        answer: answerIndex,
    ...
    const requestPayload = {
      request: {
        data: {
          nodesModified: nodesModify,
          hierarchy:
            this.competencyAssessmentSrv.getAssessmentTreeHierarchy(
    ...
    dialogRef.afterClosed().subscribe(async confirmDialogRes => {
      if (confirmDialogRes) {
        if (item === 'deleteQuestion') {
          this.competencyAssessmentSrv.deleteQuestionFromStoreData(this.selectedData.identifier, this.selectedQuestionNode, item)
        } else if (item === 'deleteSection') {
          this.competencyAssessmentSrv.deleteQuestionFromStoreData(this.contentData.identifier, this.sectionData, item)
        }
        const requestPayload = {
          request: {
            data: {
